...
THREE.Pin = function ( length, headLength, radius ) {

	THREE.Object3D.call( this );
	
	this.arrow = new THREE.ArrowHelper(new THREE.Vector3(0,1,0), new THREE.Vector3(0,-3,0), length, 0x00ff00, headLength, radius);
...
THREE.Pin.prototype.mousehit = function ( mx, my, camera, testObjects ) {
	...
	if(intersects.length > 0) {
		...
		console.log(s, focalPoint);
		//this.lookAt(focalPoint);
		this.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,1), normal);
	}
	else {
