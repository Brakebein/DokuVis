 ...
 *
 * ### Requirements
 * Add `sources.js` and `sources.css` to your `index.html` and add `dokuvis.sources` as dependency.
 *
...
.factory('Source', ['$resource', 'ApiParams', 'ApiSource', 'moment',
	function ($resource, ApiParams, ApiSource, moment) {
		...
		return $resource(ApiSource + '/:id', angular.extend({ id: '@id', type: '@type' }, ApiParams), {
			...
			/**
			 * Save spatialize properties via manual or DLT method.
			 * ```
			 * source.$spatialize({ method: <string> })
			 *   .then(function (result) {...});
			 * ```
			 * @ngdoc method
			 * @name Source#$spatialize
			 * @param method {Object} Object with `method` property. Should be `manual` or `DLT`.
			 */
			spatialize: {
				method: 'PUT',
				url: ApiSource + '/:id/spatial'
			},
			getSpatial: {
				method: 'GET',
 ...
 * ```
  */
.directive('sourcesList', ['$rootScope', '$state', 'SourcesCache', 'SourceUploader',
	function ($rootScope, $state, SourcesCache, SourceUploader) {

		return {
			templateUrl: 'components/dokuvis.sources/sourcesList.tpl.html',
			restrict: 'E',
			scope: {
				showUpload: '='
			},
